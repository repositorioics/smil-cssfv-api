...
public class JwtUtil {
	...
	public boolean validateToken(String authToken) {
		try {
			...
			Jws<Claims> claims = Jwts.parser().setSigningKey(JwtSecurityConstants.SECRET).parseClaimsJws(authToken);
			return true;
		} catch (SignatureException | MalformedJwtException | UnsupportedJwtException | IllegalArgumentException ex) {
			throw new BadCredentialsException("INVALID_CREDENTIALS", ex);
		} catch (ExpiredJwtException ex) {
			throw ex; //new Exception(ex.getHeader(), ex.getClaims(), "Token has Expired", ex);
		}
	}
	...
	public List<SimpleGrantedAuthority> getRolesFromToken(String authToken) {
		...
		Claims claims = Jwts.parser().setSigningKey(JwtSecurityConstants.SECRET).parseClaimsJws(authToken).getBody();
